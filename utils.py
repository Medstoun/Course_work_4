...
def create_data(app, db):
	with app.app_context():
		...
		u1 = User(name="sergey", password="sergey", email="sergey@ya.ru", surname="Ivanov")
		u2 = User(name="vanya", password="vanya", email="vanya@ya.ru", surname="Petrov")
		u3 = User(name="katya", password="katya", email="olga@ya.ru", surname="Smirnova")
...
def check_keys(data: dict, allowed_keys: set):
	...
	if not keys == allowed_keys:
		raise Exception('Неверные ключи')
